"""작업 G-1 5-seed 재집계 — diagnose_G1_disentangle.py가 seed별로 남긴 aggregate_s<seed>.json을
읽어 5 seed(2024/2025/2026/2027/2028) 평균±표준편차로 재집계·판정한다(학습/추론 없음, 순수 취합).

전제: 각 seed에 대해 `python analysis/diagnose_G1_disentangle.py --seed <s>`를 이미 실행해
  results/Paderborn/diag_G1_disentangle/aggregate_s<s>.json 이 존재해야 한다.
  (seed 2026은 게이트에서 이미 실행됨. 2024/2025/2027/2028은 4-seed 학습 완료 후 실행.)

산출: reports/report_G1_disentangle_5seeds.md + results/Paderborn/diag_G1_disentangle/summary_5seeds.json
재집계 항목(사용자 요청): S_shape/S_amp/S_total, ρ(RMS,score), 정상 FPR(진폭군별), fault군별 AUROC.
"""
import argparse
import json
import os
...
DIAG_DIR = os.path.join(PROJECT_ROOT, "results", "Paderborn", "diag_G1_disentangle")
REPORT_PATH = os.path.join(PROJECT_ROOT, "reports", "report_G1_disentangle_5seeds.md")
...
def main():
    ...
    # ---- 리포트 ----
    L = []
    L.append("# 작업 G-1 5-seed 재집계 — Dual-branch Shape/Amplitude Disentangle")
    L.append("")
    L.append(f"seed {used_seeds} ({len(used_seeds)} seed) mean±std. 무학습 재추론 seed별 aggregate 취합. "
             "G-1 구조·score fusion·loss 가중치 무변경(1-seed와 동일). window 2048, no-meta LOSO 4×6=24 fold/seed. "
             "S_total=val-normal 표준화 등가중 합, raw=B3 flow_NLL paired. (test 라벨 튜닝 없음.)")
    if missing:
        L.append("")
    ...
    L.append("## 핵심 요약 (5-seed mean±std)")
    L.append("")
    L.append(f"- **전체 AUROC**: S_total {fmt_ms(*o['auroc_total'][:2])} / **S_shape {fmt_ms(*o['auroc_shape'][:2])}** / "
             f"S_amp {fmt_ms(*o['auroc_amp'][:2])} vs raw {fmt_ms(*o['auroc_raw'][:2])} (gate 0.696).")
    ...
    L.append(f"- **④⑤ S_total 결합·paired**: Δ(total−raw) zero {fmt_ms(*zt['delta_total_raw'][:2])} / "
    ...
    L.append(f"- **ρ(RMS,·)**: zero S_shape {fmt_ms(*zt['rho_shape'][:2])} / S_amp {fmt_ms(*zt['rho_amp'][:2])} / "
             f"S_total {fmt_ms(*zt['rho_total'][:2])} vs raw {fmt_ms(*zt['rho_raw'][:2])} (B raw≈0.96).")
    L.append("")
    ...
    L.append("## 2) fault군별 branch AUROC (5-seed mean±std)")
    L.append("")
    ...
    L.append("")
    L.append("> A=amplitude-sensitive, S=shape-sensitive. 판정은 1-seed와 동일 다지표(①~⑤) 기준을 5-seed 평균으로 확인.")
    L.append("")
