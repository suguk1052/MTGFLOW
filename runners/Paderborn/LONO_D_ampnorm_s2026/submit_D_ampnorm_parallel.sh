#!/bin/bash
# ==============================================================================
# 작업 D(진폭 confound 교정, Route 2) 병렬 제출 — 순차 afterok 체인 대체.
#  - 24 fold 각각을 독립 잡(dependency 없음)으로 제출 → n16·n17 각 3 GPU에 packing(6 동시).
#  - node round-robin(n17,n16,n17,…). 각 잡 = (학습(main.py --amp_normalize) && test(test.py)).
#  - 이미 완료된 fold(paderborn_per_bearing_metrics.json 존재)는 자동 스킵.
#  실제 제출: bash runners/Paderborn/LONO_D_ampnorm_s2026/submit_D_ampnorm_parallel.sh
#  미리보기:  DRY_RUN=1 bash runners/Paderborn/LONO_D_ampnorm_s2026/submit_D_ampnorm_parallel.sh
#  게이트 진단(전체 완료 후): conda run -n mtgflow python analysis/diagnose_amp_correction.py
# ==============================================================================
set -euo pipefail
...
# submit 순서(B3와 동일: split 4 × LONO 6)
...
for split in "${SPLITS[@]}"; do
  for lono in "${LONOS[@]}"; do
    run_name="ampnorm_${split}_LONO${lono}"
    train="$DIR/run_Paderborn_ampnorm_${split}_LONO${lono}_5seeds.sh"
    test_rel="$DIR/run_Paderborn_ampnorm_${split}_LONO${lono}_test_5seeds.sh"
