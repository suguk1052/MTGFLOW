#!/bin/bash
# ==============================================================================
# LOSO 전반 진단(static/measured 공통, LONO6 추가): val_loss<->test_AUROC 괴리(gap)가
# 123to0만의 현상인지 나머지 LOSO split(023to1/013to2/012to3) 전반의 현상인지
# 확정하기 위한 재실행(일반성 확인용 LONO6 추가분). --log_test_auroc로 epoch별 test AUROC 기록.
# 기존 LONO_C2_5seeds / LONO_C2_measured_5seeds 결과는 덮어쓰지 않음(_lossobs3s 접미사).
# ==============================================================================

RUN_NAME="CA_raw_vib_023to1_LONO6_measured_lossobs3s"
